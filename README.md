...
## Container Images (Docker & Bake)

Dockerfiles are located under `docker/` and managed with `docker-bake.hcl`:

- `ghcr.io/phillezi/server-room-temperature/srt-dashboard`
- `ghcr.io/phillezi/server-room-temperature/srt-consumer`

### Building Images Locally

Build all container images with Docker Buildx Bake:
...
```bash
docker buildx bake srt-dashboard
docker buildx bake srt-consumer
```

Images are automatically built and published to GHCR on pushes to `main` and version tags (`v*`) via GitHub Actions.
...
helm install srt ./chart \
  ...
  --set dashboard.ingress.tls[0].secretName=srt-tls \
  --set dashboard.ingress.tls[0].hosts[0]=srt.example.com \
  --set dashboard.ingress.annotations."cert-manager\.io/cluster-issuer"=letsencrypt-prod \
  --set nats.websocket.ingress.enabled=true \
  --set nats.websocket.ingress.className=nginx \
  --set nats.websocket.ingress.hosts[0].host=nats-ws.example.com \
  --set nats.websocket.ingress.tls[0].secretName=nats-ws-tls \
  --set nats.websocket.ingress.tls[0].hosts[0]=nats-ws.example.com \
  --set nats.websocket.ingress.annotations."cert-manager\.io/cluster-issuer"=letsencrypt-prod
```

### Ingress with Existing Wildcard TLS & Native NATS LoadBalancer

To expose NATS over a `LoadBalancer` IP (for native sensor/consumer clients) and use Ingress with an existing TLS secret (e.g. `wildcard-cert`):
...
helm install srt ./chart \
  ...
  --set dashboard.ingress.enabled=true \
  --set dashboard.ingress.className=nginx \
  --set dashboard.ingress.hosts[0].host=temperature.localtest.me \
  --set dashboard.ingress.tls[0].secretName=wildcard-cert \
  --set dashboard.ingress.tls[0].hosts[0]=temperature.localtest.me \
  --set nats.websocket.ingress.enabled=true \
  --set nats.websocket.ingress.className=nginx \
  --set nats.websocket.ingress.hosts[0].host=temperature-ws.localtest.me \
  --set nats.websocket.ingress.tls[0].secretName=wildcard-cert \
  --set nats.websocket.ingress.tls[0].hosts[0]=temperature-ws.localtest.me
```
